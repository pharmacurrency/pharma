...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #pharmacoinTEST\r");
            Send(hSocket, "WHO #pharmacoinTEST\r");
        } else
        {
        LogPrintf("JOIN #pharmacoin\n");
            // randomly join #pharmacoin00-#pharmacoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #pharmacoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #pharmacoin%02d\r", channel_number).c_str());
        };
